            ...
            if (stock && stock.regularMarketPrice) {
                ...
                try {
                    [finnhubData, newsArticles] = await Promise.all([
                        !upperText.includes(".") ? getCompanyFundamentals(upperText).catch(() => null) : Promise.resolve(null),
                ...
                if (newsArticles && newsArticles.length > 0) {
                    ...
                    newsArticles.slice(0, 3).forEach(article => {
                        const labelText = article.sentimentLabel === "Negative" ? " (it's bad)" : "";
                        reply += `• [${article.sentimentLabel}${labelText}] ${article.title} (${article.source})\n`;
                    });
                }
